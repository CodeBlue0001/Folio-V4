...
export const ContactSection = ({ isDark = true, userLocation, viewers, locationError }: ContactSectionProps) => {
  ...
  const contactVis = content.visibility?.contact;
  const showGlobe = contactVis ? contactVis.showHoloEarth !== false : true;
  const showInfo = contactVis ? contactVis.showContactInfoCards !== false : true;
  const showSocials = contactVis ? contactVis.showSocialLinks !== false : true;
    ...
    <section id="contact" className={`relative min-h-screen py-12 sm:py-16 md:py-20 px-4 md:px-6 ${isDark
      ...
      <div className="container mx-auto max-w-5xl">
        ...
        */}
        ...
        {(showInfo || showSocials) && (
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
            viewport={{ once: true }}
            className={`grid gap-5 sm:gap-6 md:gap-8 mx-auto ${
              showInfo && showSocials ? 'grid-cols-1 md:grid-cols-2 max-w-4xl' : 'max-w-xl'
            }`}
          >
            {showInfo && (
