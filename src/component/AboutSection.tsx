...
export const AboutSection = ({ isDark = true }: AboutSectionProps) => {
  ...
  return (
    <section id="about" className={`relative min-h-screen flex items-center justify-center py-12 sm:py-16 md:py-20 px-4 md:px-6 overflow-hidden ${
      isDark 
        ...
        {[...Array(8)].map((_, i) => (
          <motion.div
            ...
            className={`absolute w-3 h-3 rounded-full ${
              isDark ? 'bg-emerald-400/20' : 'bg-sky-500/25'
            }`}
            style={{
              left: `${10 + (i * 12)}%`,
              top: `${20 + (i * 8)}%`,
                ...
                {timelineItems.map((item, index) => (
                  ...
                  >
                    ...
                    <motion.div
                      ...
                      className={`absolute left-2.5 w-3 h-3 bg-green-500 rounded-full border-2 ${
                        isDark ? 'border-[#0c1a2e]' : 'border-white'
                      }`}
                    />
                    ...
                    <div className={`${
                      ...
                      <div className={`text-xs sm:text-sm mb-1 ${
                        isDark ? 'text-amber-400' : 'text-green-700'
                      }`}>{item.year}</div>
              ...
              <div className={`relative overflow-hidden rounded-2xl border-2 ${
                ...
                <motion.div
                  ...
                  className={`absolute inset-0 border-2 rounded-2xl ${
                    isDark ? 'border-blue-400/30' : 'border-blue-500/40'
                  }`}
                />
              </div>
            </motion.div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              {features.map((feature, index) => (
                ...
                >
                  ...
                  <h3 className={`text-base sm:text-lg md:text-xl mb-2 transition-colors ${
                    isDark ? 'text-slate-100 group-hover:text-white group-hover:drop-shadow-[0_0_6px_rgba(255,255,255,0.6)]' : 'text-slate-800 group-hover:text-sky-600'
                  }`}>
                    {feature.title}
                  </h3>
