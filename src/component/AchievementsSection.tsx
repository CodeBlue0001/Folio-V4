...
export const AchievementsSection = ({ isDark = true }: AchievementsSectionProps) => {
  ...
  const lcVis = content.visibility?.achievements;
  const showLeetCode = lcVis ? lcVis.showLeetCodeDashboard !== false : true;
  const showStats = lcVis ? lcVis.showLeetCodeStats !== false : true;
  const showBadges = lcVis ? lcVis.showLeetCodeBadges !== false : true;
  const showViewAll = lcVis ? lcVis.showViewAllButton !== false : true;
            ...
            <div className="relative z-10">
              {lcLoading ? (
                ...
                <div className="space-y-6">
                  ...
                  {/* Flexible Layout: Both, Badges-Only, or Stats-Only */}
                  {showStats && showBadges ? (
                    ...
                    <div className="w-full">
                      <div className={`rounded-2xl border p-5 sm:p-6 ${isDark ? 'bg-slate-800/40 border-slate-700/40' : 'bg-white/50 border-slate-200'}`}>
                        <h4 className={`text-sm font-bold uppercase tracking-wider mb-4 flex items-center justify-between ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                          <span className="flex items-center gap-2">
                            <Zap className="w-4 h-4 text-amber-400" />
                            LeetCode Badges & Accomplishments
                          </span>
                          {lcData && lcData.badges.length > 0 && (
                            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${isDark ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-amber-100 text-amber-800'}`}>
                              {lcData.badges.length} Badges Earned
                            </span>
                          )}
                        </h4>
                          ...
                          <div className="flex flex-wrap gap-4 p-2">
                            {lcData.badges.map((badge, i) => (
                              <motion.div
                                ...
                                whileHover={{ y: -4, scale: 1.08 }}
                                title={badge.displayName}
                                className={`relative w-[70px] h-[70px] sm:w-[80px] sm:h-[80px] rounded-2xl border flex flex-col items-center justify-center p-2.5 transition-all group cursor-pointer ${isDark
                                  ? 'bg-slate-800/90 border-amber-500/30 hover:border-amber-400 hover:shadow-[0_0_25px_rgba(245,158,11,0.3)]'
                                  : 'bg-white border-amber-200 hover:border-amber-400 hover:shadow-xl'
                                  }`}
                              >
                                {badge.icon ? (
                                  <img src={badge.icon} alt={badge.displayName} className="w-11 h-11 object-contain rounded-lg transition-transform duration-300 group-hover:scale-110" />
                                ) : (
                    ...
                    <div className={`p-6 rounded-2xl border text-center ${isDark ? 'bg-slate-800/30 border-slate-800 text-slate-400' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>
                      <p className="text-xs font-mono">LeetCode stats & badges hidden via Admin Layout Manager.</p>
                    </div>
                  )}
